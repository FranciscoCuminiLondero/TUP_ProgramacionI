...
void OrdenarPorApellido(char clientes[8][6][50]);
void MostrarLista(char clientes[8][6][50]);

int main()
{
    char clientes[8][6][50], dniABuscar[50], opciones;
    int posicion;
    ...
    do
    {
        ...
        printf("Quiere ver la lista ordenada por apellido? S/N ");
        scanf(" %c", &opciones);

        if (opciones == 'S' || opciones == 's')
        {
            OrdenarPorApellido(clientes);
            MostrarLista(clientes);
        }

    } while (strlen(dniABuscar) != 0);
...
void OrdenarPorApellido(char clientes[8][6][50])
{
    ...
    for (int i = 0; i < 7; i++)
    {
        ...
        for (int j = i + 1; j < 8; j++)
        {
            if (strcmp(clientes[j][2], clientes[pos_menor][2]) < 0)
            {
                pos_menor = j;
            }
            for (int k = 0; k < 5; k++)
            {
                strcpy(aux, clientes[i][k]);
                strcpy(clientes[i][k], clientes[pos_menor][k]);
                strcpy(clientes[pos_menor][k], aux);
            }
        }
    }
}
